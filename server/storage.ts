...
export class DatabaseStorage {
  ...
  async getCampaigns(userId: number, limit?: number): Promise<schema.Campaign[]> {
    const results = await db.select().from(schema.campaigns).where(eq(schema.campaigns.userId, userId)).orderBy(desc(schema.campaigns.createdAt));
  ...
  async getDashboardData(userId: number, timeRange?: string) {
    try {
      ...
      // Mock data for now - replace with actual metrics calculation
