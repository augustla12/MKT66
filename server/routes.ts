...
async function doRegisterRoutes(app: express.Express): Promise<HttpServer> {
    ...
    apiRouter.post('/alerts/:id/read', async (req: AuthenticatedRequest, res, next) => { try { const success = await storage.markAlertAsRead(parseInt(req.params.id), req.user!.id); if (!success) return res.status(404).json({ error: 'Alerta não encontrado.' }); res.json({ success: true }); } catch(e){ next(e); }});
    apiRouter.post('/alerts/mark-all-read', async (req: AuthenticatedRequest, res, next) => { try { await storage.markAllAlertsAsRead(req.user!.id); res.json({ success: true }); } catch(e){ next(e); }});
