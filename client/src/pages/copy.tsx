...
- Para e-mails, estruture com parágrafos curtos e CTA claro.`;
        ...
        if (!apiKey) throw new Error("Chave da API OpenRouter não configurada no frontend.");
        ...
        if (!response.ok) { const errorData = await response.json().catch(() => ({error: {message: `Erro ${response.status} na API OpenRouter.`}})); throw new Error(`Erro da IA: ${errorData?.error?.message || response.statusText}`); }
        const result = await response.json();
        if (result.choices?.[0]?.message?.content) {
            const generatedData = JSON.parse(result.choices[0].message.content) as BackendGeneratedCopyItem;
  ...
  const generateContentIdeasMutation = useMutation<string[], Error, { product: string; audience: string; objective: string }>({
    mutationFn: async (payload) => {
      ...
      if (!response.ok) { const errorData = await response.json().catch(() => ({error: {message: 'Erro API OpenRouter'}})); throw new Error(`Erro da IA: ${errorData?.error?.message || response.statusText}`); }
      const result = await response.json();
      if (result.choices?.[0]?.message?.content) { const parsedResult = JSON.parse(result.choices[0].message.content); return parsedResult.contentIdeas || []; }
